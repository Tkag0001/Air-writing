...
# Streamlit layout
st.write("# Camera")
frame_output = st.empty() 
save_url = st.sidebar.text_input("URL folder to save:")
...
if 'save_url' not in st.session_state:
    st.session_state.save_url = ""
if 'label_img' not in st.session_state:
    st.session_state.label_img = ""
if 'folder_img' not in st.session_state:
    st.session_state.folder_img = ""
...
def main():
    ...
    # Previous pose hand of KeypointClassifier ##############################
    previous_pose = None

    # Save word image #########################################
    word_data_path = 'data/custom_data_Air_writing'
    ...
    # Board for writing ###############################################
    board = new_board(cap_width, cap_height)
    ...
    if update_url_button:
        if save_url == "" or save_url == None:
            st.sidebar.warning('This url is empty', icon="⚠️")
        else:
            st.session_state.save_url = save_url
            check_url(st.session_state.save_url)  
            st.session_state.folder_img = check_url(os.path.join(st.session_state.save_url, 'images'))
            ...
            if (len(custom_img)):
                ...
                logging.info(f"Last img: {last_img}")
                st.session_state.next_index_img = int(re.search(r'(\d+)', last_img).group(1))+1
                logging.info(f"Session_state next index: {st.session_state.next_index_img}")

    # Process 'label' button to save image #####################################
    if label_button:
    ...
    while True:
        ...
        # Draw bouding box for word:
        board2view, point1_to_crop, point2_to_crop = find_contour_word(board)
        word_image = board[point1_to_crop[1]: point2_to_crop[1], 
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if hand_sign_id == 4:  # Write
                    previous_pose = hand_sign_id
                    end_point = landmark_list[8]
                    if(start_point != None):
                        board = cv.line(board, start_point, end_point, 255, 7)
                        st.session_state.board = board
                    start_point = end_point
                    point_history.append(landmark_list[8])
                elif hand_sign_id != previous_pose:
                    if hand_sign_id == 3: # Delete
                        board = new_board(cap_width, cap_height)
                        start_point = None
                    previous_pose = hand_sign_id
        ...
        merged_image = cv.add(debug_image, board_resized) # merge 2 image
        frame = np.concatenate((board_resized, merged_image), axis=1)
        frame_output.image(frame, channels = "BGR")
...
def check_url(url):
    ...
    if not path.exists():
        ...
        logging.info(f"URL: {path} created successfully!")
    else:
        logging.info(f"URL: {path} already exists!")
    return path

def find_contour_word(img):
    contours, _ = cv.findContours(img, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
    img_to_find = img.copy()
    if len(contours)!=0:
        cnt = contours[0]
        ...
        if (point2[0] + 10 < img_to_find.shape[0]): point2[0] = point2[0] + 10
        if (point2[1] + 10 < img_to_find.shape[1]): point2[1] = point2[1] + 10

        image_contour = cv.rectangle(img_to_find, point1, point2, (255, 255, 255), 2)
        return image_contour, point1, point2
    return img_to_find, [0,0], [img.shape[0], img.shape[1]]
